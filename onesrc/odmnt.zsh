...
target=$(gum choose --header "🔍 Where do you want to mount Onedrive?" \
...
case "$target" in
  "📁 Default mount point ($DEFAULT)")
    dir="$DEFAULT"
    gum confirm "Onedrive will be mounted at: $dir. Proceed?" || exit 0
    ;;
  "🛠️  Enter custom mount point")
...
gum spin --title "🚀 Mounting Onedrive..." -- rclone mount onedrive:/ "$dir" &

# 🟢 Done message
gum style --foreground 212 --border normal --padding "1 2" --margin "1 0" \
  -- "✅ Onedrive mounted successfully to: $dir"
