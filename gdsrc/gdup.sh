...
foldert=$(gum input --placeholder "my-drive-folder" --prompt "🎯 Enter the target folder on Google Drive (or leave empty to quit):")
if [ -z "$foldert" ]; then
  echo "👋 Exited by user."
  exit 0
fi
...
gum confirm "Ready to upload '$folderl' to 'gdrive:$foldert' with flag '$flag'? Proceed?" || {
  echo "👋 Upload cancelled."
  exit 0
}

# 🚀 Upload with rclone
gum spin --title "🚀 Uploading to Google Drive..." -- rclone copy "$folderl" gdrive:"$foldert" $flag

# ✅ Done
gum style --foreground 212 --border normal --padding "1 2" --margin "1 0" \
  -- "✅ Upload completed successfully to gdrive:$foldert"
